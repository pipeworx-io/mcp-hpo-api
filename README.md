...
`tools/list` at `https://gateway.pipeworx.io/hpo-api/mcp` returns the tools in the table
...
```bash
curl -X POST https://gateway.pipeworx.io/v1/tools/term \
  -H 'Content-Type: application/json' \
  -d '{"id":"HP:0000118"}'
```

No account needed for the first calls. Inspect any tool: `GET https://gateway.pipeworx.io/v1/tools/term`. Find one: `POST https://gateway.pipeworx.io/v1/tools/search_packs` with `{"query":"..."}`.
